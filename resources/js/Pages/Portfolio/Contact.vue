...
<template>
    ...
    <PortfolioLayout :profile="profile">
        <div class="contact-page has-atmosphere">
            ...
            <motion.aside :initial="{ opacity: 0, x: -28 }" :animate="{ opacity: 1, x: 0 }" :transition="{ duration: .55 }">
                ...
                <motion.div class="contact-scene" :initial="{ opacity: 0, y: 16 }" :animate="{ opacity: 1, y: 0 }" :transition="{ delay: .15, duration: .6 }">
                    <AmbientScene tone="contact" variant="knot" />
                </motion.div>
                <div class="contact-details">
                    <motion.div :while-hover="{ x: 4 }">
                        <small>Email</small>
                        <a :href="`mailto:${profile?.email}`">{{ profile?.email }}</a>
                    </motion.div>
                    <motion.div v-if="profile?.whatsapp_number" :while-hover="{ x: 4 }">
                        <small>WhatsApp</small>
                        <a :href="`https://wa.me/${profile.whatsapp_number.replace(/\D/g, '')}`" target="_blank" rel="noreferrer">Chat on WhatsApp ↗</a>
                    </motion.div>
                    <motion.div :while-hover="{ x: 4 }">
                    ...
                    <motion.div :while-hover="{ x: 4 }">
                        ...
                        <span class="available"><i></i>{{ profile?.availability }}</span>
                    </motion.div>
                </div>
            </motion.aside>
